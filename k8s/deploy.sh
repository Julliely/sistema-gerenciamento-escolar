...
if [ "$ACTION" = "apply" ]; then
    ...
    echo "[4/6] Deploy dos microsservicos..."
    ...
    echo "[5/6] Deploy do frontend..."
    kubectl apply -f 07-frontend.yaml

    echo "[6/6] Configurando Ingress..."
    ...
    echo "============================================"
    echo ""
    echo "Verificar pods (e em qual node cada um esta):"
    echo "  kubectl get pods -n $NAMESPACE -o wide"
    echo ""
    echo "Verificar nodes do cluster (devem ser 3):"
    echo "  kubectl get nodes"
    echo ""
    echo "Verificar logs de um servico:"
    echo "  kubectl logs -n $NAMESPACE -l app=cadastro-service -f"
    echo ""
    echo "Verificar estado do Circuit Breaker:"
    echo "  kubectl exec -n $NAMESPACE deploy/gateway-service -- curl -s localhost:8080/actuator/health"
    echo ""
